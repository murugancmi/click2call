 ...
 * Telecmi-SDK v1.0.0
 * Audio Call library
 * Copyright: 2016-2017 Murugan Pandian <murugan@telecmi.com> (https://github.com/telecmi)
 * Homepage: http://telecmi.com
 * License: TeleCMI licience
 */
...
window.Telecmi=function(){var self=this;Telecmi.prototype.audioTag=function(){var audio_tag=document.createElement("AUDIO");audio_tag.setAttribute("autoplay",true);audio_tag.setAttribute("hidden",true);audio_tag.setAttribute("id","telecmi_audio_tag");document.body.appendChild(audio_tag)};Telecmi.prototype.startAgent=function(username,password,domain){var reg=this;this.audioTag();if(this.verto)if(this.verto.isConnected()||this.verto.isRegistered())this.verto.unregister();this.verto=new JsSIP.UA({uri:"sip:"+
username,ws_servers:domain,password:password,"register_expires":120,"secure_transport":false,register:true,stun_servers:["stun:stun.l.google.com:19302"]});this.verto.start();this.verto.on("connected",function(){self.onStatus({event:"ws",status:"open"})});this.verto.on("disconnected",function(){self.onStatus({event:"ws",status:"closed"})});this.verto.on("registered",function(){self.onStatus({event:"register",status:true})});this.verto.on("registrationFailed",function(){self.onStatus({event:"register",
status:false})});this.verto.on("unregistered",function(){self.onStatus({event:"register",status:false})});this.verto.on("newRTCSession",function(e){if(e.originator=="remote"){if(self.telecmi_call)return;self.telecmi_call=e.session;var call=e.session;var uri=call.remote_identity.uri;var display_name=call.remote_identity.display_name||uri.user;self.telecmi_call.on("addstream",function(e){var remoteAudio=document.getElementById("telecmi_audio_tag");remoteAudio.src=window.URL.createObjectURL(e.stream)});
self.telecmi_call.on("ended",function(e){self.onStatus({event:"hangup",status:true});if(self.telecmi_call)self.telecmi_call=null});self.telecmi_call.on("accepted",function(e){self.onStatus({event:"connected",status:true})});self.telecmi_call.on("failed",function(e){self.onStatus({event:"destroy",uri:uri.user,cause:e,status:true});if(self.telecmi_call)self.telecmi_call=null});self.onStatus({event:"ringing",from:display_name,uri:uri.user,status:true})}})};var eventHandlers={"progress":function(e){self.onStatus({event:"trying",
status:true})},"failed":function(e){self.onStatus({event:"hangup",status:true});if(self.telecmi_call)self.telecmi_call=null},"confirmed":function(e){self.onStatus({event:"connected",status:true});var localStream=self.telecmi_call.connection.getLocalStreams()[0];dtmfSender=self.telecmi_call.connection.createDTMFSender(localStream.getAudioTracks()[0])},"addstream":function(e){var remoteAudio=document.getElementById("telecmi_audio_tag");remoteAudio.src=window.URL.createObjectURL(e.stream)},"ended":function(e){self.onStatus({event:"hangup",
status:true});if(self.telecmi_call)self.telecmi_call=null}};Telecmi.prototype.call=function(inetno){if(inetno){if(this.verto){var options={"eventHandlers":eventHandlers,mediaConstraints:{audio:true,video:false},pcConfig:{"iceServers":[{"urls":["stun:stun.l.google.com:19302"]}]},mediaConstraints:{audio:true,video:false},rtcOfferConstraints:{offerToReceiveAudio:1,offerToReceiveVideo:0}};self.telecmi_call=this.verto.call("sip:"+inetno,options)}return}return};Telecmi.prototype.hangup=function(){if(self.telecmi_call){self.verto.terminateSessions();
self.telecmi_call=null}};Telecmi.prototype.answer=function(){if(self.telecmi_call){var options={"eventHandlers":eventHandlers,"mediaConstraints":{audio:true,video:false},pcConfig:{"iceServers":[{"urls":["stun:stun.l.google.com:19302"]}]},rtcOfferConstraints:{offerToReceiveAudio:1,offerToReceiveVideo:0}};self.telecmi_call.answer(options)}};Telecmi.prototype.onStatus=function(data){};Telecmi.prototype.key=function(key){var key=key|0;if(self.telecmi_call)self.telecmi_call.sendDTMF(key.toString());return};
Telecmi.prototype.mic=function(){if(self.telecmi_call)if(self.telecmi_call.isMuted().audio){console.log("its unmute");self.telecmi_call.unmute({audio:true,video:false})}else{console.log("its mute");self.telecmi_call.mute({audio:true,video:true})}return};Telecmi.prototype.logout=function(){if(self.verto)self.verto.unregister();return};Telecmi.prototype.startC2C=function(){this.audioTag();this.verto=new JsSIP.UA({uri:"sip:telecmi_c2c@c2cws.telecmi.com",ws_servers:"wss://c2cws.telecmi.com",password:"telecmi_c2c",
"register_expires":120,stun_servers:["stun:stun.l.google.com:19302"]});this.verto.on("connected",function(){self.onStatus({event:"ws",status:"open"})});this.verto.on("disconnected",function(){self.onStatus({event:"ws",status:"closed"})});this.verto.on("registered",function(){self.onStatus({event:"register",status:true})});this.verto.on("registrationFailed",function(){self.onStatus({event:"register",status:false})})};Telecmi.prototype.startCHUB=function(){this.audioTag();this.verto=new JsSIP.UA({uri:"sip:telecmi@chubws.telecmi.com",
ws_servers:"wss://chubws.telecmi.com",password:"telecmi","register_expires":120,stun_servers:["stun:stun.l.google.com:19302"]});this.verto.on("connected",function(){self.onStatus({event:"ws",status:"open"})});this.verto.on("disconnected",function(e){self.onStatus({event:"ws",status:"closed"})});this.verto.on("registered",function(){self.onStatus({event:"register",status:true})});this.verto.on("registrationFailed",function(e){self.onStatus({event:"register",status:false})});this.verto.on("unregistered",
function(){self.onStatus({event:"register",status:false})});this.verto.start()}};
